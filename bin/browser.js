...
const callChrome = async () => {
    ...
    try {
        ...
        for (let scenarioAction of scenario) {
            const scenarioOptions = scenarioAction.options;
                ...
                switch (scenarioAction.type) {
                    ...
                    case 'screenshot':
                        await page.screenshot({
                            path: request.options.screenshotsDir + '/' + scenarioOptions.name.replace('%url%', page.url().replaceAll('/', '-')) + '.png',
                        });
                        break;
                    case 'select':
                        await page.select(scenarioOptions.selector, scenarioOptions.value);
                        break;
                    case 'type':
                    ...
                    case 'waitForSelector':
                        await page.waitForSelector(scenarioOptions.selector, {
                            hidden: scenarioOptions.hidden,
                            visible: scenarioOptions.visible,
                        });
                }
            } catch (e) {
                e.message = `Error during scenario action ${JSON.stringify(scenarioAction)}: ` + e.message;
